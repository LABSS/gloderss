...
public class IntermediaryOrg extends AbstractAgent implements IIntermediaryOrg {
   ...
   * @return none
   */
  public IntermediaryOrg( Integer id, EventSimulator simulator, IntermediaryOrgConf conf, Map<Integer, ConsumerAgent> consumers, Map<Integer, EntrepreneurAgent> entrepreneurs ) {
    super( id, simulator );
    
    this.consumers = consumers;
    this.entrepreneurs = entrepreneurs;
    
    this.timeToAffiliatePDF = PDFAbstract
        .getInstance( conf.getTimeToAffiliatePDF() );
  ...
  public void receiveAffiliationRequest( AffiliateRequestAction action ) {
    ...
    Event event = new Event(
        this.simulator.now() + this.timeToAffiliatePDF.nextValue(), this,
  ...
  private void affiliate() {
    ...
    if ( !this.affiliateQueue.isEmpty() ) {
      ...
      if ( !this.affiliated.contains( entrepreneurId ) ) {
        ...
        // Output
        AbstractEntity outputEntity = OutputController.getInstance()
            .getEntity( EntityType.INTERMEDIARY_ORGANIZATION );
        outputEntity.setValue(
