...
public class ConsumerAgent extends CitizenAgent
    implements IConsumer, NormEnforcementListener {
  ...
  public ConsumerAgent( Integer id, EventSimulator simulator, ConsumerConf conf ) {
    ...
    this.entrepreneurRep = new EntrepreneurRating(
        conf.getReputationConf().getEntrepreneurRep() );
    ...
    // SANCTION SANCTION
    sancSanction = new SanctionSanction( this.id,
  ...
  public void
      setEntrepreneurs( Map<Integer, EntrepreneurAgent> entrepreneurs ) {
  ...
  public void buyProduct() {
    ...
    if ( this.normative
        .getNorm( Norms.BUY_FROM_NOT_PAYING_ENTREPRENEURS.ordinal() )
        .getStatus().equals( NormStatus.GOAL ) ) {
    ...
    if ( selectedEntrepreneurId > -1 ) {
      ...
      // Normative
      Message newMsg;
      if ( this.entrepreneurRep.getReputation(
          selectedEntrepreneurId ) < this.reputationEntrepreneurThreshold ) {
  ...
  private void loggingConsumers() {
    ...
    outputEntity.setValue(
        ConsumerOutputEntity.Field.SALIENCE_BUY_FROM_NOT_PAYING_ENTREPRENEURS
            .name(),
        this.normative.getNormSalience(
            Constants.Norms.BUY_FROM_NOT_PAYING_ENTREPRENEURS.ordinal() ) );
    
    outputEntity.setValue(
        ConsumerOutputEntity.Field.SALIENCE_BUY_FROM_PAYING_ENTREPRENEURS
            .name(),
        this.normative.getNormSalience(
            Constants.Norms.BUY_FROM_PAYING_ENTREPRENEURS.ordinal() ) );
    
    outputEntity.setValue(
        ConsumerOutputEntity.Field.SALIENCE_NOT_PAY_EXTORTION.name(),
        this.normative
            .getNormSalience( Constants.Norms.NOT_PAY_EXTORTION.ordinal() ) );
    
    outputEntity.setValue(
        ConsumerOutputEntity.Field.SALIENCE_PAY_EXTORTION.name(), this.normative
            .getNormSalience( Constants.Norms.PAY_EXTORTION.ordinal() ) );
    
    outputEntity.setValue(
        ConsumerOutputEntity.Field.SALIENCE_NOT_DENOUNCE.name(), this.normative
    ...
    if ( (msg.getSender() != this.id)
      ...
      if ( content instanceof DeliverProductAction ) {
        ...
        if ( contentMsg instanceof DenouncePunishmentAction ) {
          ...
          // Reputation
          this.entrepreneurRep.updateReputation(
              (DenouncePunishmentAffiliatedAction) contentMsg );
  ...
  public void handleObservation( Message msg ) {
    ...
    if ( (msg.getSender() != this.id)
      ...
      if ( content instanceof AffiliationAcceptedAction ) {
        ...
        Message newMsg;
        if ( this.entrepreneurRep.getReputation(
            entrepreneurId ) < this.reputationEntrepreneurThreshold ) {
        ...
        this.entrepreneurRep.updateReputation(
            (NotDenouncePunishmentAffiliatedAction) content );
        ...
        this.entrepreneurRep
            .updateReputation( (StatePunishmentAction) content );
  ...
  public void handleEvent( Event event ) {
    ...
    switch ( (String) event.getCommand() ) {
      ...
      case Constants.TAG_CONSUMER_REPUTATION_ENTREPRENEUR_THRESHOLD:
        if ( change != null ) {
          this.reputationEntrepreneurThreshold = Double
