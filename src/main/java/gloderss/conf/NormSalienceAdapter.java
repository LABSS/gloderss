...
public class NormSalienceAdapter extends
		XmlAdapter<NormsSalienceConf, Map<Integer, NormSalienceConf>> {
	...
	public NormsSalienceConf marshal(Map<Integer, NormSalienceConf> value)
		...
		for(Entry<Integer, NormSalienceConf> entry : value.entrySet()) {
			...
			normSalienceConf.setInvocationCompliance(entry.getValue()
					.getInvocationCompliance());
			normSalienceConf.setInvocationViolation(entry.getValue()
